...
async def txt_handler(bot: Client, m: Message):  
    ...
    try:
        ...
        # ✅ Auto Topic Creation (UG Bot Modification)
        import random
        topic_title = f"{random.choice(['[', '('])}{name1[:50]}{random.choice([']', ')'])}"
        try:
            topic = await bot.create_forum_topic(int(channel_id), topic_title)
            topic_id = topic.id
        except Exception as e:
