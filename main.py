...
async def download_and_extract_appx_zip(url, out_name_base, session=None, timeout=60):
    import cloudscraper, requests, subprocess, os
    tmpdir = tempfile.mkdtemp(prefix="appxzip_")
    try:
        zip_temp_path = os.path.join(tmpdir, "archive.zip")
        try:
            scraper = cloudscraper.create_scraper()
            with scraper.get(url, stream=True, timeout=timeout) as r:
                r.raise_for_status()
                with open(zip_temp_path, "wb") as fout:
        ...
        with zipfile.ZipFile(zip_temp_path, 'r') as zf:
            for zi in zf.infolist():
                ...
                if name.endswith('/'):
                    continue
                lower = name.lower()
                if any(lower.endswith(ext) for ext in ('.mp4','.mkv','.mov','.ts','.webm','.m4v','.m4a','.mpg','.mpeg','.m3u8')):
                    found_media.append((zi.file_size, name, lower.endswith('.m3u8')))
            if not found_media:
                parsed = parse_qs(urlparse(url).query)
                if 'URLPrefix' in parsed:
                    try:
                        b64 = parsed['URLPrefix'][0]
                        decoded = base64.b64decode(b64).decode('utf-8', errors='ignore')
                        return await download_and_extract_appx_zip(decoded, out_name_base, session=session, timeout=timeout)
                    except Exception:
                        pass
                raise Exception("No media found in zip")
            found_media.sort(reverse=True, key=lambda x: x[0])
            chosen_name = found_media[0][1]
            is_m3u8 = found_media[0][2]
            zf.extract(chosen_name, tmpdir)
            chosen_path = os.path.join(tmpdir, chosen_name)
        final_output = os.path.join(tmpdir, f"{out_name_base}.mp4")
        if is_m3u8:
            ffmpeg_cmd = ["ffmpeg","-hide_banner","-loglevel","error","-y","-protocol_whitelist","file,http,https,tcp,tls","-i",chosen_path,"-c","copy",final_output]
            rc = subprocess.call(ffmpeg_cmd)
            if rc!=0 or not os.path.exists(final_output):
                raise Exception("ffmpeg failed for m3u8 in zip")
            return final_output
        else:
            shutil.move(chosen_path, final_output)
            return final_output
    ...
    try:    
        ...
        for i in content:
            ...
            try:
                ...
                extracted_video = await download_and_extract_appx_zip(url, safe_name)
                await bot.send_document(chat_id=channel_id, document=extracted_video, caption=cczip)
                try:
                ...
                await bot.send_message(channel_id, f"⚠️ Zip processing failed: {str(e)}
URL: {url}")
                failed_count += 1
                count += 1
                continue
    ...
    try:
        for i in range(arg-1, len(links)):
            ...
            # Force ALL urls through proxy
            if "hls-proxy-7x6f.onrender.com" not in url:
